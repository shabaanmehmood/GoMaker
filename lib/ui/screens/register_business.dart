...
class _RegisterBusinessState extends State<RegisterBusiness> {
  ...
  TextEditingController ownerNameCtl = new TextEditingController();
  ...
  TextEditingController professionCtl = new TextEditingController();
  TextEditingController shortDescriptionnCtl = new TextEditingController();
    ...
    return Container(
      ...
      child: ListView(
        children: [
          ...
          globalWidgets.myTextField(TextInputType.name, ownerNameCtl, false, "Your Name" ),
          SizedBox(height: 10,),
          ...
          SizedBox(height: 10,),
          globalWidgets.myTextField(TextInputType.name, professionCtl, false, "Business Name" ),
          SizedBox(height: 10,),
          globalWidgets.myTextField(TextInputType.multiline, addressCtl, false, "Business Address" ),
          SizedBox(height: 10,),
          globalWidgets.myTextField(TextInputType.multiline, shortDescriptionnCtl, false, "Short description" ),
          SizedBox(height: 10,),
          globalWidgets.myTextField(TextInputType.phone, contactNumberCtl, false, "Your Phone" ),
          SizedBox(height: 20,),
  ...
  signUpButton(BuildContext context,){
    return GestureDetector(
      onTap: () async {
        ...
        else if(contactNumberCtl.text.isEmpty){
          GlobalWidgets.showErrorToast(
              context, "Phone Required", 'Provide your phone number');
        }
        else if(addressCtl.text.isEmpty){
          GlobalWidgets.showErrorToast(
              context, "Address Required", 'Provide your address');
        }
        else if(professionCtl.text.isEmpty){
          GlobalWidgets.showErrorToast(
              context, "Profession Required", 'Provide your profession');
        }
        else if(shortDescriptionnCtl.text.isEmpty){
          GlobalWidgets.showErrorToast(
              context, "Short Description Required", 'Provide your short description');
        }
        else if(!GlobalWidgets.validateEmail(emailCtl.text)){
          GlobalWidgets.showErrorToast(context, "Email not valid", 'Please provide valid email');
        }
  ...
  Future<String> checkIfEmailExists(String email, Map<String, dynamic> userInfo, BuildContext context) async {
    ...
    if(documents.isEmpty) {
      ...
      var collection = FirebaseFirestore.instance.collection('business');
  ...
  saveDataOnServer(BuildContext context) async{
    ...
    userInfo['name'] = ownerNameCtl.text;
    userInfo['email'] = emailCtl.text;
    userInfo['password'] = passwordCtl.text;
    userInfo['city'] = cityValue;
    userInfo['owner_name'] = ownerNameCtl.text;
    userInfo['address'] = addressCtl.text;
    userInfo['contact_number'] = contactNumberCtl.text;
    userInfo['profession'] = professionCtl.text;
    userInfo['short_description'] = shortDescriptionnCtl.text;
